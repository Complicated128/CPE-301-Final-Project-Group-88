...
volatile unsigned int waterThreshold = 30; // value to change
...
void handleInterrupt()
{
   bool buttonState = digitalRead(INTERRUPT_PIN);
   ...
   if (buttonState) {
      ...
      if (!buttonPressed) {
         buttonPressed = true;
         startTime = millis();
         // Don't set interruptBtn = true yet, wait for release
      }
   } else {
      // Button released (falling edge)
      if (buttonPressed) {
         interruptBtn = true;  // Now process the complete press cycle
      }
   }
}
